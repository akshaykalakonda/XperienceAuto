...
<body>
	...
	<div class="centered">
		<h1>
			<b>Requests Raised by Customers</b>
		</h1>
		<br>
				...
				<tbody>
					<c:forEach items="${rdata}" var="r">
						...
						<tr>
							...
							<td>${r.status}</td>
							<td><a
								href='<c:url value="setstatus?id=${r.id}&status=Accept"></c:url>'><button
										class="btn btn-success"
										style="font-weight: bold; color: black; border-radius: 5px;">Accept</button></a>
								<a
								href='<c:url value="setstatus?id=${r.id}&status=Reject"></c:url>'><button
										class="btn btn-danger">Decline</button></a> <a
								href='<c:url value="setstatus?id=${r.id}&status=Completed"></c:url>'><button
										class="btn btn-warning">Closed</button></a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
